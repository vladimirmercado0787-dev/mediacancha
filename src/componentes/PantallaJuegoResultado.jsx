...
export default function PantallaJuegoResultado({ resultado, onNuevo, onInicio }) {
  ...
  const ganador = totalA === totalB ? null : (totalA > totalB ? resultado.nombreA : resultado.nombreB)
    ...
    <div style={{ minHeight: '100vh', position: 'relative', fontFamily: C.font, background: '#08090c', color: C.texto }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 480, margin: '0 auto', padding: '22px 14px 40px' }}>
        <div style={{ textAlign: 'center', marginBottom: 6 }}>
          <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase', ...ORO }}>Final del juego</span>
        </div>
        ...
        {ganador && <div style={{ textAlign: 'center', fontSize: 14, fontWeight: 700, marginTop: -4, marginBottom: 14, ...ORO }}>🏆 Ganó {ganador}</div>}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => onInicio && onInicio()} style={{ flex: 1, borderRadius: 13, padding: 15, border: '1px solid rgba(255,255,255,.14)', background: 'transparent', color: C.texto, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>Ir al inicio</button>
          <button onClick={() => onNuevo && onNuevo()} style={{ flex: 1, borderRadius: 13, padding: 15, border: 'none', background: T.boton, color: '#1a1205', fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>Juego nuevo</button>
        </div>
      </div>
    </div>
  )
